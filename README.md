...
### 🔍 一键扫描清理
- **17种垃圾分类**：覆盖 Windows 临时文件、系统缓存、浏览器缓存、回收站、Windows 更新缓存、传递优化文件、Microsoft Defender 防病毒非关键文件、缩略图缓存、DirectX 着色器缓存、日志文件、内存转储、旧 Windows 安装、应用缓存等
...
- **系统清理项对齐**：快速和深度扫描覆盖 Windows 清理向导中的 Defender `LocalCopy/Support`、各用户缩略图缓存、传递优化缓存和 D3D 着色器缓存；隔离区、Defender 定义库与其他系统关键目录继续排除
...
- **操作记录**：删除、彻底删除、恢复和解除防复活等操作会以 JSON Lines 记录到当前配置数据目录的 `logs/shell_icons.log`，模块内可一键打开查看。
- **温和刷新**：使用 Windows Shell 变更通知重新读取外壳节点，不结束 `explorer.exe`，减少对 TranslucentTB 等任务栏增强工具的影响。
- **统一数据清理**：设置中的“清空本地数据”会单独列出虚拟磁盘操作记录和注册表备份，可按需清理；清理日志和注册表备份总项不会重复统计这些专属数据。
...
| 传递优化文件 | 🟡 中等 | Windows 更新和应用分发产生的可重建缓存 |
| Microsoft Defender 防病毒 | 🟡 中等 | Defender 的非关键日志和临时文件，不包含隔离区和病毒库 |
| 缩略图缓存 | 🟢 安全 | 文件资源管理器的缩略图缓存 |
| DirectX Shader 缓存 | 🟢 安全 | GPU 着色器编译缓存，删除后会自动重建 |
